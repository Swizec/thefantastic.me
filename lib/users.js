...
module.exports = {
    ...
    delete: function (user_id) {
        redis.hget('user:'+user_id, 'email', function (err, email) {
            redis.multi()
                .lrem('users', 0, user_id)
                .hdel('user:'+user_id, 'email')
                .hdel('user:'+user_id, 'password')
                .hdel('user:'+user_id, 'index')
                .hdel('user:'+user_id, 'token')
                .hdel('user:'+user_id, 'token_secret')
                .hdel('email-id', email)
                .srem('emails', email)
                .exec(function (err) {
                });
